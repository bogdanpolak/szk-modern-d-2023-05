...
| `git checkout --track origin/osoba1` | Stworzenie lokalnej gałęzi |
...
| `git reset --hard` | wyczyszczenie zmian roboczych oraz zmian w poczekalni |

| Polecenie | Opis |
| --- | --- |
| `git reset -- {nazwa pliku}` | Usunięcie pliku z poczekalni |
| `git reset --hard` | Skasowanie wszystkich zmian w katalogu roboczym oraz w poczekalni |
| `git reset HEAD~2` | cofnięcie aktualnej gałęzi o 2 rewizje bez zmiany plików roboczych |
...
* Sourcetree - Atalasian - https://www.sourcetreeapp.com/ *(BitBacket account)*
...
### Wariant z spakowaną liniową historią

Squash and merge to strategia w Git scalania małych, samodzielnych rewizji w jedną większą rewizję. Dzięki takiemu podejściu historia zmian jest bardziej przejrzysta. Połączone małe commity ułatwiają cofanie zmian. Gałąź główna (`master` / `main`) pozostaje czysta i zawiera tylko istotne zmiany.

Wykonanie squash spowoduje, że wszystkie nasze commity z gałęzi roboczej zostaną połączone w jeden commit i zostaną wprowadzone do gałęzi głównej:

```
git merge --squash feature
```
Stworzenie nowej rewizji złączonymi zmianami:
```
