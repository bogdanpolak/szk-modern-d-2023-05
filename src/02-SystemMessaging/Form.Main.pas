...
type
  TTaskHelper = class helper for TTask
    class procedure SleepWithCancelation(const aMiliseconds: integer;
      const aTask: ITask);
  end;

class procedure TTaskHelper.SleepWithCancelation(const aMiliseconds: integer;
  const aTask: ITask);
var
  sw: TStopwatch;
begin
  ...
  fTask := TTask.Create(
    ...
    begin
      ...
      begin
        ...
        TTask.SleepWithCancelation(20 + Random(2000), fTask);
      end;
      fTask := nil;
    end);
  fTask.Start;
...
begin
  StatusBar1.Panels[0].Text := DataModuleMain.GetMessageAvgInterval.ToString;
end;
