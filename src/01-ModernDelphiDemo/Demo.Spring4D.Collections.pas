unit Demo.Spring4D.Collections;
...
class procedure TSpringCollectionsDemo.RunEmployeeCollection(const aConsole
  : IConsole);
var
  allEmployees: IList<TEmployee>;
  employees: IEnumerable<TEmployee>;
  emp: TEmployee;
  boss: TEmployee;
begin
  allEmployees := TCollections.CreateList<TEmployee>(True);
  allEmployees.Add(TEmployee.Create('Jan Kowalski', 9800));
  allEmployees.Add(TEmployee.Create('Tomasz Jankowski', 6300));
  allEmployees.Add(TEmployee.Create('Ewa Tomaszewska', 8500));
  allEmployees.Add(TEmployee.Create('Prezes', 21500));
  allEmployees.Add(TEmployee.Create('Wice Prezes', 15000));
  ...
  employees := allEmployees.Where(
    function(const e: TEmployee): boolean
    begin
      Result := e.Salary < 10000;
    end);

  if (allEmployees.TryGetFirst(boss,
    function(const e: TEmployee): boolean
    begin
      Result := e.Salary > 20000;
    end)) then
  begin
    aConsole.Writeln('Mega Boss: %s %s', [boss.FirstName, boss.LastName]);
  end;
...
type
  ...
  strict private
    class function BuildWeather(const aLocation: string;
...
class function TWeatherGenerator.BuildWeather(const aLocation: string;
  const data: TArray<Variant>): Shared<TWeather>;
begin
